...
class ParamOpt(AbstractOpt):
    ...
    "-f file1 file2'. Supports multiple parameter values as indicated by the 'multi_type'
    and 'count' fields. There are three multi option types, as indicated by the
    multi_type constructor arg, which is a MultiTypeEnum enum.
    ...
    and - assuming no other defined options - the positional params would be ['p1', 'p2'].
    Note that for this type - if the command line doesn't contain the required number
    of params, then it is a parse error. E.g., this is a parse error, if the param is
    defined as EXACTLY 3:

    "my-utility --files foo bar"

    AT_MOST
    For this kind, the parser attempts to pull up to the specified count of parameters
    and the presence of an option terminates consumption of tokens from the command line.
    ...
    If the multi-type is not specified in the yaml, then it defaults to EXACTLY 1. If the type
    is EXACTLY 1 - either by fallback as described or by explicit initialization, then
    ...
    def _ensure_data_type(self, values):
        ...
        in conformance - or b) the object has no defined type, or c) there are no values
        to validate. Returns False if the object has a defined type and the passed values
