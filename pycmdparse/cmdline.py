...
class CmdLine:
    ...
    6) If there is an error parsing the command line, use the class to display
       the errors, or, display usage instructions as defined in the yaml
    """
    ...
    def _parse(cls, cmdline_stack):
        ...
        # A callback can be defined in the subclass to perform customized validation of
        # positional params - and - individual options on the command line. The function
        # must return a tuple: element zero is an OptAcceptResultEnum value, and element
        # one is an error message to display to the user if element zero is 'ERROR'
